...
class QuestionQuerySet(QuerySet):
	"""
	A basic ''QuerySet'' subclass, provides query functionality and some helper
	methods for an intuitive interface.
	"""
	
	def active(self, **kwargs):
		"""
		A utility method that filters results based on ''Question''
		models that are only ''Active''.
		"""
		group = False
		if kwargs.get('group'):
			group = kwargs['group']
		...
		if kwargs.get('slug'):
			...
			return self.exclude(status__exact=enums.STATUS_INACTIVE, ).order_by('topic')
		else:
			return self.filter(status__exact=enums.STATUS_ACTIVE, )
